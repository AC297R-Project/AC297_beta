...
def get_market(params, invalid):
	with open('markets.txt', 'r') as f:
		for line in f:
			market = line.replace("'", " ").replace(" ", "").strip().split(',')
			market = [m for m in market if len(m) > 0]
...
def simulated_annealing(hedge, init_temp, min_temp, cool_by, reanneal, num_iter,
	...
	for i in range(num_iter):
		...
		if np.isnan(betas[1:]).any():
			print '*****Beta', betas
			print '******p', hedge.dollar_portfolio_sum_ret
			print '******m',hedge.dollar_market_sum_ret
			print '******mnames', hedge.market
